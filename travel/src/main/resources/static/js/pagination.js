...
(function(){
    ...
    Pagination.prototype = {
        render: function(obj){
            ...
            this._total = obj.total;//总的数据条数
            this._pagesize = obj.pagesize;//每页的条数
        ...
        bindEvent: function(){
            ...
            function creatBtns(ismorePosition){
                ...
                if(ismorePosition=="all"){
                    // console.log('省略前后都有');
                    var number = parseInt((this._btnsValue-3)/2);
                    var halfnum = number;//左右两边个数
                    html += firstbtn + ismore;
                    console.log(html);
                    //                  7 - 2 = 5
                    console.log("左右两边个数"+halfnum)
                    console.log("当前页"+this._currentPage)



                    var current = parseInt(this._currentPage);
                    console.log("i:"+(current- number))
                    var number2 = (this._btnsValue-3)%2;
                    console.log("i<=:"+current+number+(number2))
                    for(var i = (current-halfnum);i<=current+halfnum+(number2);i++){
                        if(i == this._currentPage){
                            html += '<li class="pagenum pagination-btn pagination-current" data-num='+i+'>'+i+'</li>'
                        }else{
                            html += '<li class="pagenum pagination-btn" data-num='+i+'>'+i+'</li>'
